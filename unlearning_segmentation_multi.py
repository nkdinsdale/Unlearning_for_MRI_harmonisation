...
# Unlearning main for the segmentation model with multiple outputs
########################################################################################################################
from models.unet_model_multiple_unlearning import UNet, segmenter, domain_predictor
...
from train_utils_segmentation import train_unlearn_multi, val_unlearn_multi, train_encoder_unlearn_multi, val_encoder_unlearn_multi
...
y_oasis = np.reshape(y_oasis, (-1, 128, 128, 128, 4))
...
optimizer_step1 = optim.Adam(list(unet.parameters()) + list(segmenter.parameters()) + list(domain_pred.parameters()), lr=1e-5)
...
for epoch in range(args.epoch_reached, args.epochs+1):
    if epoch < args.epoch_stage_1:
        ...
        loss, acc, dm_loss, conf_loss = train_encoder_unlearn_multi(args, models, train_dataloaders, optimizers, criterions, epoch)
        torch.cuda.empty_cache()  # Clear memory cache
        val_loss, val_acc = val_encoder_unlearn_multi(args, models, val_dataloaders, criterions)
        ...
        loss, acc, dm_loss, conf_loss = train_unlearn_multi(args, models, train_dataloaders, optimizers, criterions, epoch)
        torch.cuda.empty_cache()  # Clear memory cache
        val_loss, val_acc = val_unlearn_multi(args, models, val_dataloaders, criterions)
