...
# Semisupervised learning for segmentation
########################################################################################################################
from models.unet_2d_model import UNet, segmenter, domain_predictor
...
if args.channels_first:
    X_biobank= np.transpose(X_biobank, (0, 3, 1, 2))
    y_biobank= np.transpose(y_biobank, (0, 3, 1, 2))
    print('CHANNELS FIRST')
    print('Biobank Data shape: ', X_biobank.shape)
    print('Labels shape: ', y_biobank.shape)
    X_oasis = np.transpose(X_oasis, (0, 3, 1, 2))
    y_oasis = np.transpose(y_oasis, (0, 3, 1, 2))
...
X_oasis_int = np.copy(X_oasis)
d_oasis_int = np.copy(d_oasis)

num_samples = 10
print('Number of OASIS samples being used = ', num_samples)
X_oasis = X_oasis[:num_samples]
y_oasis = y_oasis[:num_samples]
d_oasis = d_oasis[:num_samples]

X_oasis = np.repeat(X_oasis, int(len(X_biobank)/num_samples), axis=0)
X_oasis = np.reshape(X_oasis, (-1, 1, 128, 128))
y_oasis = np.repeat(y_oasis, int(len(X_biobank)/num_samples), axis=0)
y_oasis = np.reshape(y_oasis, (-1, 4, 128, 128))
d_oasis = np.repeat(d_oasis, int(len(X_biobank)/num_samples), axis=0)
d_oasis = np.reshape(d_oasis, (-1, 2))
...
b_int_train_dataset = numpy_dataset(X_btrain, d_btrain)
b_int_val_dataset = numpy_dataset(X_bval, d_bval)
...
train_dataloaders = [b_train_dataloader, o_train_dataloader, b_int_train_dataloader, o_int_val_dataloader]
...
for epoch in range(args.epoch_reached, args.epochs+1):
    if epoch < args.epoch_stage_1:
        ...
        loss, acc, dm_loss, conf_loss = train_encoder_domain_unlearn_semi(args, models, train_dataloaders, optimizers, criterions, epoch)
        torch.cuda.empty_cache()  # Clear memory cache
        val_loss, val_acc = val_encoder_domain_unlearn_semi(args, models, val_dataloaders, criterions)
        ...
        loss, acc, dm_loss, conf_loss = train_unlearn_semi(args, models, train_dataloaders, optimizers, criterions, epoch)
        torch.cuda.empty_cache()  # Clear memory cache
        val_loss, val_acc = val_unlearn_semi(args, models, val_dataloaders, criterions)
